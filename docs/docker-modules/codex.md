...
Codex caches login details locally at `~/.codex/auth.json`. The default `compose.yaml` mounts `${HOME}/docker/.codex` to `/home/user/.codex` so credentials can persist across containers.
